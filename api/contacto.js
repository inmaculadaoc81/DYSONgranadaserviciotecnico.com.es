...
module.exports = async (req, res) => {
  ...
  try {
    ...
    await transport.sendMail({
      from: `"DyMaster" <${process.env.SMTP_USER}>`,
      to: process.env.CONTACT_EMAIL || process.env.SMTP_USER,
      replyTo: email,
      subject: 'Nueva consulta - DyMaster Servicio Técnico Dyson Granada',
      text: `Nombre: ${nombre}\nModelo: ${modelo || '-'}\nEmail: ${email}\nTeléfono: ${telefono}\n\n${mensaje}`
    });
    return res.status(200).json({ok:true});
  } catch (error) {
    console.error('Error al enviar consulta de DyMaster', error);
